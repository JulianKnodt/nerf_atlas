...
dnerf_gru: clean
	python3 runner.py -d data/data/bouncingballs/ --data-kind dnerf --render-size 64 \
...
dnerfae: clean
	...
	--crop --epochs 80_000  --save models/djj_ae_gamma.pt --model ae --crop --batch-size 2 \
	--crop-size 32 --near 2 --far 6 -lr 5e-4 --no-sched --valid-freq 499 \
	--dnerfae --time-gamma --load models/djj_ae_gamma.pt --omit-bg --loss-window 750 \
  #--serial-idxs
...
unisurf: clean
	python3 -O runner.py -d data/nerf_synthetic/lego/ --data-kind original \
	--render-size 64 --crop --epochs 80_000 --save models/lego_us.pt \
